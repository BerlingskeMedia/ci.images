...
* `use-existing-task-definition` - If set on `true` it tells the plugin to ignore task settings and try to use existing task definition from ECS. `existing-task-definition` must be defined. Default is `false`.
