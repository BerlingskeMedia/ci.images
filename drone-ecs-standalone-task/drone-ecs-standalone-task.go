...
func main() {
	app := cli.NewApp()
	app.Name = "AWS ECS Run Standalone Task"
	app.Usage = "AWS ECS Run Standalone Task"
	app.Action = run
	...
	app.Flags = []cli.Flag{
		...
		cli.BoolFlag{
			...
			Usage:  "Use existing task definition for running standalone task",
			EnvVar: "PLUGIN_USE_EXISTING_TASK_DEFINITION",
		},
		cli.StringFlag{
			Name:   "existing-task-definition-arn",
			Usage:  "ARN of task definition to use for running standalone task",
			EnvVar: "PLUGIN_EXISTING_TASK_DEFINITION_ARN",
		},
	}
